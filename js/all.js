...
$('.moreBtn').on('click', function() {
	...
	if (moreBoxClass.search('hidden') !== -1) {
		$("#tags")[0].classList = 'm-width moreBox show';
	} else {
			...
			if (data.code === 200) {
				if (data.data.length !== 0) {
					...
					for (var tls = 0; tls < arr.length; tls++) {
						...
						if (arr[tls].messageType !== 1) {
							talkLists += '<li style="margin-bottom: 10px;"><div style="display: flex;margin: 0  0 10px;">' +
								...
								arr[tls].summary + '</div>' +
								'</div><p style="left: 40px;">' + CurentTime(new Date(arr[tls].createtime)) + '</p></li>';
						} else {
							...
							if (!fileFormats.match(/.png|.jpg|.gif|.jpeg/)) {
								talkLists +=
									...
									'</a></div><p style="left: 40px;">' + CurentTime(new Date(arr[tls].createtime)) + '</p></li>';
							} else {
					...
					if(data.data[0].adminname != '' ){
						if($('.useNum').html() == '' || $('.useNum').html() != data.data[0].adminname ){
							$('.useNum').html(data.data[0].adminname)
						}
					}
				}
						...
						if (arrs[tls].state === 1) {
							if (arrs[tls].messageType != 1) {
								talkLists += '<li style="margin-bottom: 10px;"><div style="display: flex;margin: 0  0 10px;">' +
									...
									'</div><p style="left: 40px;">' + CurentTime(new Date(arrs[tls].createtime)) + '</p></li>';
							} else {
								...
								if (!fileFormats.match(/.png|.jpg|.gif|.jpeg/)) {
									talkLists +=
										...
										'</video></div>' +
										'</a></div><p style="left: 40px;">' + CurentTime(new Date(arrs[tls].createtime)) + '</p></li>';
								} else {
									talkLists += '<li style="overflow: hidden;"><div style="display: flex;margin: 0  0 10px;">' +
										'<img src="' + photoPath + arrs[tls].adminpic +
										'" alt="" class="userPic">' +
							...
							if (arrs[tls].messageType != 1) {
								talkLists += '<li style="overflow: hidden;"><div class="fr" style="display: flex;margin: 0  0 10px;">' +
									...
									appPic.user.cLogo +
									'" alt="" class="userPic">' +
									'</div><p style="right: 40px;">' + CurentTime(new Date(arrs[tls].createtime)) + '</p></li>';
							} else {
								...
								if (!fileFormat.match(/.png|.jpg|.gif|.jpeg/)) {
									talkLists +=
										...
										appPic.user.cLogo +
										'" alt="" class="userPic">' +
										'</a></div><p style="right: 40px;">' + CurentTime(new Date(arrs[tls].createtime)) + '</p></li>';
								} else {
									talkLists +=
						...
						if (arr[tls].state === 1) {
							...
							if (arr[tls].messageType != 1) {
								talkLists += '<li style="overflow: hidden;"><div style="display: flex;float: right;margin: 0  0 10px;">' +
									...
									appPic.user.cLogo +
									'" alt="" class="userPic">' +
									'</div><p style="right: 40px;">' + CurentTime(new Date(arr[tls].createtime)) + '</p></li>';
							} else {
								...
								if (!fileFormat.match(/.png|.jpg|.gif|.jpeg/)) {
									talkLists +=
										...
										appPic.user.cLogo +
										'" alt="" class="userPic">' +
										'</a></div><p style="right: 40px;">' + CurentTime(new Date(arr[tls].createtime)) + '</p></li>';
								} else {
									talkLists +=
										'<li style="overflow: hidden;"><div style="display: flex;float: right;margin: 0  0 10px;word-wrap:break-word;word-break:break-all;">' +
				...
				if (data.data.realName === null) {} else {
					...
					for (let tl = 0; tl < arrs.length; tl++) {
						...
						if (arrs[tl].messageType != 1) {
							talkLists += '<li style="overflow: hidden;"><div style="display: flex;float: left;margin: 0  0 10px;">' +
								'<img src="' + photoPath +
								arrs[tl].adminpic +
								'" alt="" class="userPic">' +
								'<div class="" ' +
	...
	if ($('#mTalk')[0].value !== '') {
		...
		$.ajax({
			...
			success: function(data) {
				if (data.code === 200) {
					let talkList = '';
					talkList += '<li class="clearfix" style="margin-bottom: 10px;"><div style="display: flex;float: right;margin: 0  0 10px;">' +
						'<div class="" ' +
						'style="background-color:#bbe2f3;padding: 5px;margin-right:10px;border-radius: 6px;word-wrap:break-word;word-break:break-all;">' +
						news + '</div>' +
						'<img src="' + appPic.user.cLogo + '" alt="" class="userPic">' +
						'</div><p style="position: absolute;right: 40px; top: 1px;color: #bdbdbd;">' + CurentTime(new Date())  + '</p></li>';
			...
			if (data.code === 200) {
				...
				if (!fileFormat.match(/.png|.jpg|.gif|.jpeg/)) {
					imgs +=
						...
						'" type="video/mp4"></source></video></div>' +
						'<img src="' + appPic.user.cLogo + '" alt="" class="userPic">' +
						'</a></div><p style="right: 40px;">' + CurentTime(new Date()) + '</p></li>';
					$('.talk').append(imgs);
					gotoEnd();
				} else {
					imgs +=
						...
						'<img src="' + appPic.user.cLogo + '" alt="" class="userPic">' +
						'</div><p style="right: 40px;">' + CurentTime(new Date()) + '</p></li>';
					$('.talk').append(imgs);
					gotoEnd();
				}
			} else {
